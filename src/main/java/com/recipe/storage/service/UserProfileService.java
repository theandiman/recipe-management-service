...
public class UserProfileService {
   ...
   */
  public com.recipe.storage.dto.PagedUserSearchResponse searchUsers(
      String query, String pageToken, int pageSize, String currentUserId) {
    if (firestore == null) {
      return com.recipe.storage.dto.PagedUserSearchResponse.builder()
          .users(new ArrayList<>()).hasMore(false).build();
    }

    try {
      String q = query != null ? query.toLowerCase().trim() : "";
      QuerySnapshot snapshot = firestore.collection(usersCollection).limit(50).get().get();

      List<UserProfileResponse> results = new ArrayList<>();
      for (DocumentSnapshot doc : snapshot.getDocuments()) {
        ...
        if ("PRIVATE".equalsIgnoreCase(visibility) && !uid.equals(currentUserId)) {
          continue;
        }

        boolean matches = (displayName != null && displayName.toLowerCase().contains(q))
            || (bio != null && bio.toLowerCase().contains(q));

        if (q.isBlank() || matches) {
          try {
            results.add(getUserProfile(uid, currentUserId));
          } catch (Exception e) {
            // Ignore missing user profile
          }
        }
      }

      int fromIndex = 0;
      int toIndex = Math.min(pageSize, results.size());
      List<UserProfileResponse> paged = results.subList(fromIndex, toIndex);

      return com.recipe.storage.dto.PagedUserSearchResponse.builder()
          .users(paged)
          .hasMore(toIndex < results.size())
          .build();
   ...
   */
  public com.recipe.storage.dto.FeaturedCreatorsResponse getFeaturedCreators(
      int limit, String currentUserId) {
    if (firestore == null) {
      return com.recipe.storage.dto.FeaturedCreatorsResponse.builder()
          .creators(new ArrayList<>()).build();
    }

    try {
      QuerySnapshot snapshot = firestore.collection(usersCollection).limit(20).get().get();

      List<UserProfileResponse> creators = new ArrayList<>();
      for (DocumentSnapshot doc : snapshot.getDocuments()) {
        String uid = doc.getId();
        try {
          UserProfileResponse profile = getUserProfile(uid, currentUserId);
          if ("PUBLIC".equalsIgnoreCase(profile.getVisibility())
              && profile.getPublicRecipeCount() > 0) {
            creators.add(profile);
          }
        } catch (Exception e) {
          // Ignore
        }
      ...
      creators.sort((a, b) -> Long.compare(
          b.getFollowerCount() + b.getPublicRecipeCount(),
          a.getFollowerCount() + a.getPublicRecipeCount()));

      int max = Math.min(limit, creators.size());
      return com.recipe.storage.dto.FeaturedCreatorsResponse.builder()
          .creators(creators.subList(0, max))
          .build();
    } catch (InterruptedException | ExecutionException e) {
      log.error("Error fetching featured creators", e);
      return com.recipe.storage.dto.FeaturedCreatorsResponse.builder()
          .creators(new ArrayList<>()).build();
    }
  }
